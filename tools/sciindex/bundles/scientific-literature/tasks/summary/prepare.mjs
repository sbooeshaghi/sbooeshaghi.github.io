...
function help() {
  console.log(`Usage:
  node tools/sciindex/bundles/scientific-literature/tasks/summary/prepare.mjs --claims=local/sciindex/claims/accepted.json

Options:
  --out-dir=PATH   Default: local/sciindex/summary/inputs`);
}
...
const outDir = resolveRootPath(parsed.outDir || "local/sciindex/summary/inputs");
...
for (const paper of accepted.papers || []) {
  const packet = {
    ...
    claims: (paper.claims || []).map(({ id, statement }) => ({ id, statement })),
  };
